...
file="/scratch/kbuckowi/july2017/july2017.txt"

echo
while read -r line;
do
#Call trimmomatic, paired end, over the desired raw files (right and left reads). 
#Ouptput files are split between paired and unpaired, which have 50 in the name due to the minimum length being set there.
#Illumina adapters are removed before quality trimming occurs, adjust the ILLUMINACLIP path as needed.
trimmomatic PE -phred33 ./raw/${line}_R1_001.fastq.gz ./raw/${line}_R2_001.fastq.gz ./output/paired/${line}_50_R1p.fastq.gz ./output/unpaired/${line}_50_R1up.fastq.gz  ./output/paired/${line}_50_R2p.fastq.gz ./output/unpaired/${line}_50_R2up.fastq.gz ILLUMINACLIP:/home/joh97948/utils/TruSeq3-PE-2.fa:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:4:20 MINLEN:50;
done < "$file"
